...
class ImageTools:
    ...
    """
    @staticmethod
    def compress(image_bytes: Union[str, bytes], image_type: Union[str, None] = None, max_size: int = 1024):
    ...
    @staticmethod
    def generate_captcha(code: Union[str, None] = None, width: int = 500, height: int = 100, bg_color: tuple = None, text_color: tuple = None):
        t0 = time()
        ...
        for i in range(line_count):
            draw.line((x[i][0], y[i][0], x[i][1], y[i][1]), fill=tuple(fill[i]), width=boldness[i])
