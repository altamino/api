...
async def make_link(request: Request):
    ...
    link = await links_table.find_one({"objectId": data['objectId'], "objectType": data['objectType']})
    if link == None:
        link = ModelFabric.Construct(
            Global.Links,
            code = check['aminoId'] if data['objectType'] == 0 else Generator.RealString(8),
...
async def resolute_link(request: Request, q: str):
    ...
    q = q.replace(
            "http://aminoapps.com/", "http://altamino.top/"
        ).replace(
            "https://aminoapps.com/", "http://altamino.top/"
        ).replace(
            "http://altamino.top/u/", ""
        ).replace(
            "http://altamino.top/p/", ""
